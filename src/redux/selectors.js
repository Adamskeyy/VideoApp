...
export const videoSortSelector = createSelector(
    ...
    (videos, sortType) => {
        if (sortType === OLDEST) {
            const sortedByOldest = [...videos].sort((a, b) => new Date(a.rawDateTime).getTime() - new Date(b.rawDateTime).getTime());
            return sortedByOldest
        }
        if (sortType === NEWEST) {
            const sortedByNewest = [...videos].sort((a, b) => new Date(b.rawDateTime).getTime() - new Date(a.rawDateTime).getTime());
